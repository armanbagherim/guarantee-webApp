...
async function getProducts(guaranteeId, session) {
  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/v1/api/guarantee/client/assignedProductGuarantees/guarantee/${guaranteeId}`, {
    headers: {
      Authorization: `Bearer ${session.token}`
    }
  })
...
export default async function RepairRequest({ params }) {
  ...
  const { result: products } = await getProducts(params.id, session)
